...
class FireStoreUtils {
          ...
          if (userDocument.data() != null && userDocument.exists) {
            try {
              ...
              await firestore
                  .collection(USERS)
                  .doc(user.userID)
                  .update({"wallet_amount": user.walletAmount + double.parse(referralAmount.toString())}).then((value) => print("north"));

             await  FireStoreUtils.createPaymentId().then((value) async {
                final paymentID = value;
                await FireStoreUtils.topUpWalletAmountRefral(paymentMethod: "Referral Amount", amount: double.parse(referralAmount.toString()), id: paymentID,userId: referralModel!.referralBy,note: "You referral user has complete his this order #${orderModel.id}");
              });
  ...
  static Future<bool> sendFcmMessageMultiple(String title, String message, String token1, dynamic token2) async {
    try {
      var url = 'https://fcm.googleapis.com/fcm/send';
      var header = {
        "Content-Type": "application/json",
